...
''' it is for both amazond data ml-1m'''
...
class AmazonRatingLoader():
    def __init__(self, data_dir, item_num):
        ...
        # self.tp = pd.read_csv(os.path.join(self.data_dir,'preprocessed_data_min20.csv'))
    ...
    def split_train_test_proportion(self, data, test_prop=0.3):
        print("start train eval split!")
        np.random.seed(98765)
        data_grouped_by_user = data.groupby('user')
        tr_list, te_list = list(), list()
        print('total user', len(data_grouped_by_user))
        for i, (_, group) in enumerate(data_grouped_by_user):
            ...
            if n_items_u > 3:
                ...
                group = group.reset_index()
                idx = np.zeros(n_items_u, dtype='bool')
                choice = np.random.choice(n_items_u, size=1,\
                                            replace=False).astype('int64')
                idx[choice] = True
                tr_list.append(group[np.logical_not(idx)])
                te_list.append(group[idx])
        ...
        tr_user = data_tr.groupby('user')
        print('tr',len(tr_user))
        data_te = pd.concat(te_list)
        te_user = data_te.groupby('user')
        print('te',len(te_user))
        return data_tr, data_te
    ...
    def get_seq_data(self, df_ori):
        ...
        for i, (g_user, group) in enumerate(grouped):
            ...
            # except:
                ...
                if not os.path.exists(os.path.join(self.data_dir, f'amazon_feature_per_user')):
                    os.mkdir(os.path.join(self.data_dir, f'amazon_feature_per_user'))
                if not os.path.exists(os.path.join(self.data_dir, f'amazon_feature_per_user/period_{per+1}')):
    ...
    def load_train_data_mf(self, csv_file, csv_file_val, csv_file_te, batch_size, total_batch):
        tp = pd.read_csv(csv_file)
        tp_add = pd.read_csv(csv_file_val)
        tp_add2 = pd.read_csv(csv_file_te)

        train_user_uni = tp['uid'].unique().tolist()
        
        added = tp_add['uid'].unique().tolist()
        added2 = tp_add2['uid'].unique().tolist()
        # print(added2)
        train_user_uni = train_user_uni + added + added2
        ### sorting??
        # train_user_uni.sort()
        rows = tp['uid'].tolist() + tp_add['uid'].tolist() + tp_add2['uid'].tolist()
        cols = tp['sid'].tolist() + tp_add['sid'].tolist() + tp_add2['sid'].tolist()
        ...
        total_data = []
        for user in train_user_uni:
            for item in range(31842):
                total_data.append((user,item))
        
        neg_data = set(total_data) - set(data[1])
        data[1] = list(neg_data)
        # print(len(data[1]))
        for i in range(total_batch):
            d = {}
            d[0] = data[0][i*batch_size : (i+1)*batch_size]
            d[1] = data[1][i*batch_size*10 : (i+1)*batch_size*10]
            yield d

    @staticmethod
    def load_tr_te_data_mf(csv_file_te, batch_size):
        tp_te = pd.read_csv(csv_file_te)

        vad_te_user_uni = tp_te['uid'].unique().tolist()
        ...
        cols_te = tp_te['sid']
        matrix = sparse.csr_matrix((np.ones_like(rows_te),
                                (rows_te, cols_te)), dtype='float64',
                                shape=(len(vad_te_user_uni), 31842))
        
        total_data = []
        for user in vad_te_user_uni:
            for item in range(31842):
                total_data.append((user, item))
        
        for i in range(len(vad_te_user_uni)):
            d={}
            d[0] = matrix[i*batch_size:(i+1)*batch_size]
            d[1] = total_data[i*31842*batch_size:(i+1)*31842*batch_size]
            yield d
    ...
    def load_pretrained(self):
        for i in range(5):
            if i == 0:
                if self.data_dir == "./data/amazon":
                    self.pretrained = torch.load(f'./pretrained/amazon/h_{i+1}.pt').unsqueeze(0)
                elif self.data_dir == "./data/amazon_min20":
                    self.pretrained = torch.load(f'./pretrained/amazon_min20/h_{i+1}.pt').unsqueeze(0)
                elif self.data_dir == "./data/amazon_min10_woman":
                    self.pretrained = torch.load(f'./pretrained/amazon_min10_woman/h_{i+1}.pt').unsqueeze(0)
            else:
                if self.data_dir == "./data/amazon":
                    self.pretrained = torch.cat((self.pretrained, torch.load(f'./pretrained/amazon/h_{i+1}.pt').unsqueeze(0)), 0)
                elif self.data_dir == "./data/amazon_min20":
                    self.pretrained = torch.cat((self.pretrained, torch.load(f'./pretrained/amazon_min20/h_{i+1}.pt').unsqueeze(0)), 0)
                elif self.data_dir == "./data/amazon_min10_woman":
                    self.pretrained = torch.cat((self.pretrained, torch.load(f'./pretrained/amazon_min10_woman/h_{i+1}.pt').unsqueeze(0)),0)
...
class ItemRatingLoader():
    ...
    def load_tr_te_data_mf(csv_file_te, batch_size):
        ...
        matrix = sparse.csr_matrix((np.ones_like(rows_te),
                                (rows_te, cols_te)), dtype='float64',
                                shape=(len(vad_te_user_uni), 3355))
        ...
        for i in range(len(vad_te_user_uni)):
            d={}
            d[0] = matrix[i*batch_size:(i+1)*batch_size]
            d[1] = total_data[i*3355*batch_size:(i+1)*3355*batch_size]
            yield d
...
if __name__ == "__main__":
    dataload = AmazonRatingLoader('./data/amazon_min10_woman', 66109)
...
# if __name__ == "__main__":
    # dataset = ItemDataset_amazon('./data/amazon_min10_woman', 20)
